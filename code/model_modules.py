...
def resnet50_custom(pretrained=False, norm_type=None, image_size=[None,None], remove_maxpool_for_LN=False):
    """ 
    image_size is [H, W]
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            if norm_type == 'NN' or norm_type is None:
                ...
                setattr(model, name, nn.InstanceNorm2d(module.num_features))#, track_running_stats=True))
                ...
                for sub_name, sub_module in block.named_children():
                    if isinstance(sub_module, nn.BatchNorm2d):
                        if norm_type == 'NN' or norm_type is None:
                            ...
                            setattr(block, sub_name, nn.InstanceNorm2d(sub_module.num_features))#, track_running_stats=True))
                            #  if you want to use running statistics for normalization during evaluation, you should set track_running_stats to True. If you always want to use batch statistics for normalization, you should set track_running_stats to False.
                        elif norm_type == 'BN_Original':
                            pass
                        elif norm_type == 'BN_New':
                            setattr(block, sub_name, nn.BatchNorm2d(sub_module.num_features))
                        else:
                            raise NotImplementedError
                        ...
                        for sub_sub_name, sub_sub_module in sub_module.named_children():
                            if isinstance(sub_sub_module, nn.BatchNorm2d):
                                if norm_type == 'NN' or norm_type is None:
                                    ...
                                    setattr(sub_module, sub_sub_name, nn.InstanceNorm2d(sub_sub_module.num_features))#, track_running_stats=True))
                                elif norm_type == 'BN_Original':
                                    pass
                                elif norm_type == 'BN_New':
                                    setattr(sub_module, sub_sub_name, nn.BatchNorm2d(sub_sub_module.num_features))
                                else:
                                    raise NotImplementedError
    return model
# encoding input images into feature maps and then into embeddings, return both 
# feature map for Decoder, embedding for MLP
class EncoderCNN(nn.Module):
    def __init__(self, version=2, pretrained=False, norm_type=None, image_size=[None,None]):
        ...
        # print(len(self.net_parts))

    def get_v0(self): # original
        model = resnet50_custom(pretrained=self.pretrained, norm_type=self.norm_type, image_size=self.image_size)
        return model # N * 1000
    ...
    def get_v2(self): # without maxpooling
        model = resnet50_custom(pretrained=self.pretrained, norm_type=self.norm_type, image_size=self.image_size, remove_maxpool_for_LN=True)
        # print(model)
...
if __name__ == '__main__':
    ...
    blur_encoder1 = resnet50_custom(pretrained=True, norm_type='NN', image_size=(240, 320))
    print(blur_encoder1)
    output = blur_encoder1(input)
    print(output.shape)
    
    blur_encoder2 = resnet50_custom(pretrained=True, norm_type='LN', image_size=(240, 320))
    print(blur_encoder2)
    output = blur_encoder2(input)
    print(output.shape)
    
    blur_encoder3 = resnet50_custom(pretrained=True, norm_type='IN', image_size=(240, 320))
    print(blur_encoder3)
    output = blur_encoder3(input)
    print(output.shape)
    
    blur_encoder4 = resnet50_custom(pretrained=True, norm_type='BN', image_size=(240, 320))
    print(blur_encoder4)
